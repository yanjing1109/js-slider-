export function RoomSlider (slideId) {
...
RoomSlider.prototype.resetAllButton = function(){
    ...
    let buttonPosition = this.slideHeight/2-14;		//动态得到按钮垂直居中的top值
...
RoomSlider.prototype.navButtonClick = function(e){
    clearInterval(this.time);
    let _this= this;
    let liTarget = e.target;		//匿名函数的this问题，需要同时指向构造函数与目标dom，采用e.target获取index
    this.time = setInterval(function(){
        let left = parseInt(_this.room.style.left);
        //如果点击的原点位置大于轮播现在的位置
